...
global_locations11=list();
global_locations22=list(); 
cl <- makePSOCKcluster(nworkers)
clusterSetRNGStream(cl, c(1:nworkers))
registerDoParallel(cl)
global_locations1 = foreach(i = 1:50, .packages = c("Matrix")) %dopar%{
  center_nonzero_locations(local_locations_aggre1,threshold=0.02*i)
}
global_locations2 = foreach(i = 1:50, .packages = c("Matrix")) %dopar%{
  center_nonzero_locations(local_locations_aggre2,threshold=0.02*i)
}
stopCluster(cl)
